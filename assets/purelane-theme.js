...
function getSection(el) { return el?.closest?.(SECTION_SELECTOR) || el?.parentElement || null; }
...
function updateScrollState() {
  ...
  document.querySelectorAll('#hdr,[data-purelane-header]').forEach(hdr => hdr.classList.toggle('up', window.scrollY > 90));
  ...
  document.querySelectorAll('#scenes,[data-purelane-scenes]').forEach(scenes => {
    ...
    document.querySelectorAll('[data-scene]').forEach(section => {
      ...
      if (amount > visible) { visible = amount; best = Number.parseInt(section.dataset.scene, 10) || 1; }
    });
...
function setInspectorMode(active) { document.documentElement.classList.toggle('purelane-inspect-mode', Boolean(active)); }
...
function initHeroes(scope = document) {
  scope.querySelectorAll('.hstage').forEach(stage => {
    ...
    const state = { current: Math.max(0, slides.findIndex(x => x.classList.contains('on'))), timer: null, breath: null, paused: false, editorPaused: false };
    HERO_STATE.set(root, state);

    const syncDots = () => dots.forEach((dot, i) => {
      dot.classList.toggle('on', i === state.current);
      dot.setAttribute('aria-current', i === state.current ? 'true' : 'false');
    });
    const go = n => {
      state.current = (n + slides.length) % slides.length;
    ...
    const start = () => {
      ...
      stop(); state.timer = setInterval(() => go(state.current + 1), 4000);
    };
    ...
    if (product && window.Animation && !reduced) {
      try { state.breath = product.animate([
        { filter: 'drop-shadow(0 34px 54px rgba(2,20,19,.60))' },
        { filter: 'drop-shadow(0 42px 68px rgba(2,20,19,.72))' },
        { filter: 'drop-shadow(0 34px 54px rgba(2,20,19,.60))' }
      ], { duration: 4800, easing: 'ease-in-out', iterations: Infinity }); } catch (_) {}
    }
    go(state.current); start();

    addCleanup(root, () => { stop(); state.breath?.cancel?.(); stage.removeEventListener('pointerenter', onEnter); stage.removeEventListener('pointerleave', onLeave); HERO_STATE.delete(root); });
  });
}
...
function initRotators(scope = document) {
  scope.querySelectorAll('#rot,.rot,[data-purelane-rotator]').forEach(rot => {
    ...
    const reduced = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
    const show = n => {
      const old = current; current = (n + imgs.length) % imgs.length;
      imgs[old]?.classList.remove('on'); dots[old]?.classList.remove('on'); imgs[current]?.classList.add('on'); dots[current]?.classList.add('on');
      if (cap) { const b = cap.querySelector('b'); const s = cap.querySelector('span'); if (b) b.textContent = imgs[current]?.dataset.name || ''; if (s) s.textContent = imgs[current]?.dataset.note || ''; }
    };
...
async function addToCart(event) {
  ...
  try {
    const response = await fetch('/cart/add.js', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, body: JSON.stringify({ id, quantity: 1 }) });
...
function burgerToggle(event) {
  ...
  if (!burger) return;
  const root = getSection(burger), nav = root?.querySelector('.nav');
  if (!nav) return;
...
function closeMobileNav(event) {
  ...
  if (!link) return;
  const root = getSection(link), burger = root?.querySelector('.burger'), nav = root?.querySelector('.nav');
  if (!nav || !burger) return;
  nav.classList.remove('open'); burger.setAttribute('aria-expanded', 'false'); burger.setAttribute('aria-label', 'Open menu');
}
...
function selectHeroBlock(event) {
  ...
  if (!block) return;
  const root = getSection(block), state = HERO_STATE.get(root);
  if (!state) return;
  const slides = [...root.querySelectorAll('.hslide')], index = slides.indexOf(block);
  if (index < 0) return;
  state.editorPaused = true; state.paused = true; if (state.timer) clearInterval(state.timer); state.timer = null; state.current = index;
  ...
  dots.forEach((dot, i) => { dot.classList.toggle('on', i === index); dot.setAttribute('aria-current', i === index ? 'true' : 'false'); });
}
...
function deselectHeroBlock(event) {
  ...
  const root = getSection(block), state = HERO_STATE.get(root);
  if (!state) return;
  state.editorPaused = false; state.paused = false;
  const slides = [...root.querySelectorAll('.hslide')];
  slides.forEach((slide, i) => slide.classList.toggle('on', i === state.current));
  if (!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches && slides.length > 1) state.timer = setInterval(() => {
    state.current = (state.current + 1) % slides.length;
    slides.forEach((slide, i) => slide.classList.toggle('on', i === state.current));
  }, 4000);
}

function initialise(scope = document) { initGlobalMotion(); initReveals(scope); initHeroes(scope); initRotators(scope); initScrollHints(scope); scheduleScrollState(); }
...
document.addEventListener('shopify:section:load', event => { const root = eventSection(event); if (!root) return; cleanup(root); initialise(root); });
document.addEventListener('shopify:section:unload', event => { const root = eventSection(event); if (root) cleanup(root); scheduleScrollState(); });
document.addEventListener('shopify:section:reorder', initialise);
document.addEventListener('shopify:section:select', event => { const root = eventSection(event); if (root) { root.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); initReveals(root); } scheduleScrollState(); });
document.addEventListener('shopify:section:deselect', scheduleScrollState);
document.addEventListener('shopify:block:select', event => { selectHeroBlock(event); event.target?.scrollIntoView?.({ behavior: 'smooth', block: 'nearest' }); });
